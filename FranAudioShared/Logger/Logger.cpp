...
namespace FranAudioShared::Logger
{
	// ========================
	// Logging Functions
	// ========================
	...
	void LogError(const std::string& message, bool newLine)
	{
	#ifndef FRANAUDIO_DISABLE_LOGGING
		if (customStreamBuffer != nullptr)
		{
	...
	void LogWarning(const std::string& message, bool newLine)
	{
	#ifndef FRANAUDIO_DISABLE_LOGGING
		if (customStreamBuffer != nullptr)
		{
	...
	void LogSuccess(const std::string& message, bool newLine)
	{
	#ifndef FRANAUDIO_DISABLE_LOGGING
		if (customStreamBuffer != nullptr)
		{
	...
	void LogGeneric(const std::string& message, bool newLine)
	{
	#ifndef FRANAUDIO_DISABLE_LOGGING
		if (customStreamBuffer != nullptr)
		{
	...
	void LogSeperator(bool newLine)
	{
	#ifndef FRANAUDIO_DISABLE_LOGGING
	...
	FranAudioConsole::FranAudioConsole()
	{
		entries.reserve(512); // Pre-allocate space for 512 entries
	}

	void FranAudioConsole::Write(const std::string& entry)
	{
		/*
		if (!entries.empty())
		{
	...
	void FranAudioConsole::Clear()
	{
		entries.clear();
	}

	void FranAudioConsole::Remove(size_t index)
	{
	...
	ConsoleStreamBuffer::ConsoleStreamBuffer(FranAudioConsole& console)
		: console(console)
	{

	}

	FranAudioConsole& ConsoleStreamBuffer::GetConsole()
	{
		return console;
	}
